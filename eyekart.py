...
while not escaped:
    ...
    if count == 0:
        ...
        cv2.putText(capture, "Next, the system will open the EyeKart game", (int(width/2)-460, 380), font, 1, (255, 0, 0))
        cv2.putText(capture, "Looking at top of the screen accelerates and bottom brakes/reverses", (int(width/2)-680, 460), font, 1, (255, 0, 0))
        cv2.putText(capture, "Steer by looking left or right", (int(width/2)-310, 540), font, 1, (255, 0, 0))
        cv2.putText(capture, "Try and finish two laps as fast as possible", (int(width/2)-460, 620), font, 1, (255, 0, 0))
        cv2.imshow("EyeTrack", capture)
    ...
    #elif trained == False and not game_mode and not finished:
    if training:
        cv2.rectangle(capture, (0,0), (width, height), (255,255,255), -1)
        cv2.putText(capture, "TRAINING", (int(width/2)-450, 200), font, 5, (255, 0, 0))
        cv2.putText(capture, "Please wait a moment as the system learns your gaze", (int(width/2)-550, 300), font, 1, (255, 0, 0))
    ...
    if trained:
        ...
        #Display predicted point on screen
